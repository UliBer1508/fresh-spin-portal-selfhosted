...
const BookingChangeNotificationDialog = ({ notification, onAcknowledge }: Props) => {
  ...
  const label = notification ? CHANGE_LABELS[notification.change_type] ?? notification.change_type : "";
          ...
          <div className="space-y-3 text-base">
            ...
            {(ci || co) && (
              <p className="text-sm text-muted-foreground">
                Aufenthalt:{" "}
                {ci ? format(ci, "dd.MM.yyyy", { locale: de }) : "?"}
                {" – "}
                {co ? format(co, "dd.MM.yyyy", { locale: de }) : "?"}
              </p>
            )}
            <p className="text-sm text-muted-foreground">
